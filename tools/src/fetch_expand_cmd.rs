...
pub fn fetch_packages(base_url: &str, fetch_list:  Vec<(PackageIdent,PackageTarget)>, dst_path: &std::path::Path)
                      -> Result<()>
{
    let client = Client::new(base_url, crate::PRODUCT, crate::VERSION, Some(dst_path.clone()) ).unwrap();
...
{
    let strings: Vec<&str> = PackageTarget::supported_targets().map(|t| t.as_ref()).collect();
    return strings.join(", ");
}
